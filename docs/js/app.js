...
const data = {
  ...
  projects: [
    { id: "autoresearch", name: "autoresearch", owner: "karpathy", url: "https://github.com/karpathy/autoresearch", stars: 35057, category: "full-stack-research", desc: { en: "Single-GPU auto-research agent by Karpathy", zh: "Karpathy新作，单GPU自动研究Agent" }, tags: ["AutoML", "SingleGPU"], updated: "2026-03-15", created: "2026-03-01" },
    { id: "AI-Scientist-v2", name: "AI-Scientist-v2", owner: "SakanaAI", url: "https://github.com/SakanaAI/AI-Scientist-v2", stars: 2274, category: "full-stack-research", desc: { en: "End-to-end research automation", zh: "端到端科研自动化" }, tags: ["Multi-Agent", "End-to-End"], updated: "2026-03-14", created: "2026-03-05" },
    ...
    { id: "last30days-skill", name: "last30days-skill", owner: "mvanhorn", url: "https://github.com/mvanhorn/last30days-skill", stars: 4162, category: "literature-research", desc: { en: "Cross-platform research aggregation", zh: "跨平台研究聚合工具" }, tags: ["RAG", "Multi-Source"], updated: "2026-03-15", created: "2026-02-25" },
    ...
    { id: "scholaraio", name: "scholaraio", owner: "ZimoLiao", url: "https://github.com/ZimoLiao/scholaraio", stars: 153, category: "literature-research", desc: { en: "AI-powered academic research terminal", zh: "AI学术研究终端" }, tags: ["Terminal", "Research"], updated: "2026-03-15", created: "2026-02-20" },
    { id: "ResearchClaw", name: "ResearchClaw", owner: "ymx10086", url: "https://github.com/ymx10086/ResearchClaw", stars: 127, category: "literature-research", desc: { en: "Personal AI research assistant on OpenClaw", zh: "基于OpenClaw的个人研究助手" }, tags: ["OpenClaw", "Assistant"], updated: "2026-03-15", created: "2026-03-05" },
    ...
    { id: "airs-bench", name: "airs-bench", owner: "facebookresearch", url: "https://github.com/facebookresearch/airs-bench", stars: 66, category: "benchmark", desc: { en: "Meta's AI Research Science benchmark", zh: "Meta的AI研究基准" }, tags: ["Meta", "Benchmark"], updated: "2026-03-09", created: "2026-03-01" },
    ...
    { id: "ata", name: "ata", owner: "Agents2AgentsAI", url: "https://github.com/Agents2AgentsAI/ata", stars: 67, category: "tools", desc: { en: "End-to-end engineering/research agents", zh: "端到端工程和研究Agent" }, tags: ["Multi-Agent", "Workflow"], updated: "2026-03-11", created: "2026-03-01" }
  ],
  domains: [
...
function initTimelineChart() {
  ...
  let cum = 0;
  const cumData = months.map(m => { cum += monthlyData[m]; return cum; });
  
  chartInstance = new Chart(ctx, {
    type: 'line',
    data: {
      labels: months.map(m => { const [y,mo]=m.split('-'); return lang==='en' ? `${mo}/${y.slice(2)}` : `${mo}月`; }),
      datasets: [{
        label: lang === 'en' ? 'Total Projects' : '项目总数',
        data: cumData,
        borderColor: '#6366f1',
    ...
    options: {
      ...
      scales: {
        ...
        y: { beginAtZero: true, grid: { color: 'rgba(100,116,139,0.1)' }, ticks: { color: '#64748b', stepSize: 10 } }
      }
    }
  });
}
...
function updateChartLanguage() {
  if (chartInstance) {
    chartInstance.data.datasets[0].label = lang === 'en' ? 'Total Projects' : '项目总数';
...
function renderAll() {
  ...
  document.getElementById('topProjects').innerHTML = top.map((p, i) => `
    <div class="project-card-featured">
      ${i < 3 ? `<div class="rank-badge rank-${i+1}">${i+1}</div>` : ''}
      <div class="project-header">
        <div class="project-icon">${getCatIcon(p.category)}</div>
        <div class="project-info">
          <div class="project-name"><a href="${p.url}" target="_blank">${p.owner}/${p.name}</a></div>
          <div class="project-owner">${p.tags.join(' · ')}</div>
        </div>
      </div>
      <p class="project-desc">${p.desc[lang]}</p>
      <div class="project-footer">
        <span class="project-stars">⭐ ${formatStars(p.stars)}</span>
        <span style="font-size:0.75rem;color:var(--text-muted);">${p.updated}</span>
      </div>
    </div>
  `).join('');
...
function renderProjects() {
  ...
  document.getElementById('allProjects').innerHTML = filtered.map(p => `
    <div class="project-card">
      ...
      <p class="project-desc">${p.desc[lang]}</p>
      <div class="project-footer">
        <span style="font-size:0.75rem;">${p.tags.map(t => `<span class="tag">${t}</span>`).join(' ')}</span>
        <span style="font-size:0.75rem;color:var(--text-muted);">${p.updated}</span>
      </div>
    </div>
  `).join('');
